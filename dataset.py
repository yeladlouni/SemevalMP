...
class MPCNNDatasetFactory(object):
    ...
    def get_dataset(dataset_name, word_vectors_dir, word_vectors_file, batch_size, device):
        ...
        if dataset_name == 'sts':
            ...
            if not os.path.exists(trec_eval_path):
                raise FileNotFoundError('Semeval requires the trec_eval tool to run. Please run get_trec_eval.sh inside Castor/utils (as working directory) before continuing.')
            dataset_root = './semeval/'
            train_loader, dev_loader, test_loader = CQA.iters(dataset_root, word_vectors_file, word_vectors_dir, batch_size, device=device, unk_init=UnknownWordVecCache.unk)
            embedding = nn.Embedding.from_pretrained(CQA.TEXT_FIELD.vocab.vectors)
